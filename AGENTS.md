...
(IAM, trigger, build SA). Never edit `functions.tf` to change runtime behavior —
all behavioral config is set by `SiveraV2/functions/scripts/deploy-all.sh`.
...
terraform plan -target=google_cloudfunctions2_function.https
```
...
1. **Firestore CMEK is create-time-only.** The live `(default)` database
   ...
   set `var.firestore_cmek_key_name` — `variables.tf` defaults it to `""` and
   [firestore.tf](firestore.tf) adds `lifecycle.ignore_changes` for `type`,
   `location_id`, `cmek_config`. The KMS resources in [kms.tf](kms.tf) still
   provision a key for future (new) databases.
...
3. **Secret values must never land in Terraform state.** [secrets.tf](secrets.tf)
   ...
   [secrets.tf](secrets.tf) (`local.platform_secret_names`) and
...
Resources are split into per-domain files at the repo root. Read each file
for the full picture; the notes below cover non-obvious behavior:

| File | Non-obvious behavior |
|---|---|
| [project.tf](project.tf) | Enables all required GCP APIs. Adding a new API? Add it here, not in an ad-hoc `google_project_service` elsewhere. |
| [kms.tf](kms.tf) | CMEK keyring exists *for future use* — the live DB is not encrypted with it (see invariant #1). |
| [firestore.tf](firestore.tf) | Loads `firebase/firestore.indexes.json` + `firebase/firestore.rules` from `${path.root}/firebase/...`. Edit those files; don't add `google_firestore_index` resources by hand. |
| [iam.tf](iam.tf) | Three SAs: `sa-sync-runner` (Functions runtime), `sa-scheduler-invoker` (Scheduler→Cloud Run OIDC), `sa-vercel-app` (Vercel→Firestore + mints OIDC tokens to call `sync-on-connect`). |
| [pubsub.tf](pubsub.tf) | DLQ topic exists but the dead-letter *policy* is **not wired** — Eventarc-managed subscriptions need either an explicit `google_pubsub_subscription` or a gcloud reach-in. Tracked as a follow-up; don't assume DLQ is live. |
| [functions.tf](functions.tf) | Resolves `project_number` dynamically via `data.google_project` to grant roles to the Gen 2 build SA (`<num>-compute@developer.gserviceaccount.com`); without these roles, first build fails with "missing permission on the build service account". Stub zip source is in `stub/` at the repo root. |
| [secrets.tf](secrets.tf) | See invariant #3. `local.platform_secret_refs` is consumed by `SiveraV2/functions/scripts/deploy-all.sh`. |
| [scheduler.tf](scheduler.tf) | OIDC-authenticated POST `{}` to function HTTPS URLs. Depends on `google_cloud_run_v2_service_iam_member.scheduler_invokes_https_functions` — keep that ordering. |

Cross-domain IAM (`google_cloud_run_v2_service_iam_member`) lives in
[functions.tf](functions.tf): the Vercel SA's invoke permission on
`sync-on-connect`/`sync-on-demand` and the Scheduler SA's invoke permission
on the three scheduled functions are bound at the Cloud Run service level
because that's where Gen 2 functions live.
...
- **DLQ policy is not wired via Terraform** — `scripts/setup-dlq.sh` attaches
  ...
  after first apply. [pubsub.tf](pubsub.tf) only creates the DLQ topic; the
